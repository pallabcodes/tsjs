...
class LegacyPaymentSystem {
  async makePayment(_amountInCents: number): Promise<string> {
...
export class PaymentSystemAdapter implements ModernPaymentAPI {
  ...
  async processPayment(amount: number, _currency: string): Promise<boolean> {
    try {
...
class VideoValidator {
  async validate(_file: File): Promise<boolean> {
...
export class RateLimitedAPIClient implements APIClient {
  ...
  constructor(client: APIClient, limitPerMinute = 60) {
    this.client = client;
...
export class Container implements UIComponent {
