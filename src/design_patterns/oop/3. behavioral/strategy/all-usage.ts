...
class StripePaymentStrategy implements PaymentProcessingStrategy {
    ...
    async processPayment(details: PaymentDetails): Promise<PaymentResult> {
        try {
            ...
            const response = await this.makeStripeRequest(details);
    ...
    private async makeStripeRequest(details: PaymentDetails): Promise<unknown> {
...
class OAuth2Strategy implements AuthStrategy {
    ...
    private async performOAuthFlow(credentials: AuthCredentials): Promise<{userId: string; accessToken: string}> {
    ...
    private async performTokenRefresh(token: string): Promise<{newToken: string}> {
...
class CSVExportStrategy implements ExportStrategy {
    ...
    constructor(delimiter: string = ',', includeHeaders: boolean = true) {
...
async function executePayment(strategy: PaymentProcessingStrategy, payment: PaymentDetails): Promise<void> {
...
async function authenticateUser(strategy: AuthStrategy, credentials: AuthCredentials): Promise<void> {
...
async function exportData(strategy: ExportStrategy, data: unknown[], options: ExportOptions): Promise<void> {
