    ...
    <div className="flex-1 flex flex-col bg-[#020202] text-white/90 font-mono overflow-hidden">
      ...
      <div className="pt-20 pb-12 flex flex-col items-center px-6 bg-gradient-to-b from-[#080808] to-transparent">
        <div className="flex items-center gap-4 mb-8">
           <div className="p-3 bg-vms-accent/10 border border-vms-accent/20 rounded-2xl">
              <Sparkles size={32} className="text-vms-accent" />
           </div>
           <div>
              <h1 className="text-2xl font-bold tracking-tighter uppercase">Sovereign Semantic <span className="text-vms-accent italic">OS</span></h1>
        ...
        <form onSubmit={handleSearch} className="w-full max-w-3xl relative group">
          <div className="absolute inset-y-0 left-5 flex items-center pointer-events-none">
            <Search size={20} className="text-white/20 group-focus-within:text-vms-accent transition-colors" />
          </div>
          <input 
            type="text"
            value={query}
            ...
            className="w-full h-16 bg-[#0a0a0a] border border-white/10 rounded-2xl pl-14 pr-32 text-[14px] focus:outline-none focus:border-vms-accent focus:ring-4 focus:ring-vms-accent/5 transition-all shadow-2xl"
          />
          <div className="absolute inset-y-2 right-2 flex items-center">
            <button 
              type="submit"
              className="h-full px-6 bg-vms-accent text-black font-bold text-[11px] uppercase tracking-widest rounded-xl hover:bg-white transition-all flex items-center gap-2"
            >
        ...
        <div className="flex items-center gap-3 mt-6">
           ...
           {SUGGESTED_QUERIES.map(q => (
             <button 
               ...
               className="px-3 py-1 bg-white/[0.03] border border-white/5 rounded-full text-[9px] text-white/40 hover:text-vms-accent hover:border-vms-accent/30 transition-all"
             >
               {q}
             </button>
           ))}
        ...
        <div className="max-w-5xl mx-auto">
          {isSearching ? (
             <div className="flex flex-col items-center justify-center py-20 gap-4 opacity-50">
                <div className="w-12 h-12 border-2 border-vms-accent border-t-transparent rounded-full animate-spin" />
                <span className="text-[10px] uppercase tracking-widest text-vms-accent">Synthesizing Metadata...</span>
             </div>
          ) : results.length > 0 ? (
            <div className="flex flex-col gap-8">
               <div className="flex items-center justify-between border-b border-white/5 pb-4">
                  <div className="flex items-center gap-2">
                     <TrendingUp size={14} className="text-vms-emerald-500" />
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {results.map((res, i) => (
                    <div 
                      ...
                      className="group bg-[#080808] border border-white/5 rounded-xl overflow-hidden cursor-pointer hover:border-vms-accent/30 transition-all"
                    >
                       <div className="flex h-32">
                          {/* Thumbnail Simulation */}
                          <div className="w-48 bg-white/[0.02] relative overflow-hidden flex items-center justify-center">
                             <div className="absolute inset-0 bg-gradient-to-br from-vms-accent/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                             <Camera size={24} className="text-white/10 group-hover:text-vms-accent/40 transition-colors" />
                             <div className="absolute bottom-2 left-2 px-1.5 py-0.5 bg-black/60 text-[8px] text-vms-accent font-bold uppercase tracking-tighter">CLIP_META_{res.time}</div>
                          </div>
                          ...
                          <div className="flex-1 p-4 flex flex-col justify-between">
                             <div>
                                <div className="flex items-center justify-between mb-1">
                                   <span className="text-[10px] font-bold text-white/80 group-hover:text-vms-accent transition-colors uppercase">{res.match}</span>
                                   <div className="flex items-center gap-1 px-1.5 py-0.5 bg-vms-emerald-500/10 border border-vms-emerald-500/20 rounded">
                                      <ShieldCheck size={10} className="text-vms-emerald-500" />
                                      <span className="text-[8px] font-bold text-vms-emerald-400">{(res.score * 100).toFixed(0)}% Match</span>
                                   </div>
                                </div>
                             ...
                             <div className="flex items-center justify-between pt-2">
                                ...
                                <ChevronRight size={16} className="text-white/10 group-hover:text-vms-accent group-hover:translate-x-1 transition-all" />
                             </div>
                          </div>
                       </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 py-12">
               <div className="p-6 bg-white/[0.02] border border-white/[0.05] rounded-2xl flex flex-col gap-4">
                  <Sparkles size={24} className="text-vms-accent" />
               ...
               <div className="p-6 bg-white/[0.02] border border-white/[0.05] rounded-2xl flex flex-col gap-4">
                  <Clock size={24} className="text-vms-accent" />
               ...
               <div className="p-6 bg-white/[0.02] border border-white/[0.05] rounded-2xl flex flex-col gap-4">
                  <ShieldCheck size={24} className="text-vms-accent" />
