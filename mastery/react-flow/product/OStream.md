...
- **The Engine (The Core)**: **C / C++**.
  - _Role_: Deep-packet inspection, zero-copy buffer analysis, and NAL-unit parsing. Chosen for maximum performance and direct integration with existing C-based media drivers.
- **The Connective Tissue**: **Go** (or **Elixir** for high-concurrency signaling).
  - _Role_: Managing the OTEL collection pipeline, signaling between edge/cloud, and providing high-availability distributed coordination.
- **The Management Plane**: **Java (Spring Boot / DGS)**.
  - _Role_: Handling complex business logic, multi-tenancy, authentication, and the GraphQL API for the Shell.
- **The Analysis Layer**: **Python**.
  - _Role_: Real-time anomaly detection and predictive failure analysis using the ingested telemetry.
- **The Interface (The Shell)**: **React, TypeScript, ReactFlow**.
  - _Role_: High-fidelity, reactive visualization of the media pipeline.
