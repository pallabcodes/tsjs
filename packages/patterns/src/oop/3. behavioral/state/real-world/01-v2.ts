...
class Order {
  ...
  setState(state: OrderState): void {
    if (this.canTransitionTo(state.constructor.name)) {
      ...
      throw new Error(
        `Invalid state transition: ${this.state.getStatus()} -> ${state.getStatus()}`
      );
    }
  }
  ...
  private canTransitionTo(stateName: string): boolean {
    const allowedTransitions =
      this.transitionRules.get(this.state.constructor.name) || [];
...
// ==========================
try {
  const order = new Order(new PendingState(undefined as any), transitionRules);
  (order.getState() as PendingState).setOrder(order);
