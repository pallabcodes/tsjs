...
class EmailNotification extends NotificationTemplate {
  protected override validateRecipient(recipient: string): boolean {
  ...
  protected override deliver(recipient: string, message: string): void {
...
class PushNotification extends NotificationTemplate {
  ...
  protected override deliver(recipient: string, message: string): void {
    console.log(
      `Sending push notification to ${recipient} with message: "${message}"`
    );
...
const emailSuccess = emailNotifier.sendNotification(
  'user@example.com',
  'Hello via Email!'
);
if (!emailSuccess) {
