// Complex subsystems
class Account {
  private balance: number; // Initial balance
...
// Complex subsystems
class CreditCard {
  private limit: number; // Credit limit
  private balance: number; // Current balance on the credit card
  ...
  payOff(amount: number): void {
    // Pays off the specified amount on the credit card
    this.balance -= amount;
  }

  checkCreditCardBalance(): number {
...
// Facade class
class BankingFacade {
  private account: Account; // Instance of Account
  private creditCard: CreditCard; // Instance of CreditCard
  ...
  makeDeposit(amount: number): void {
    ...
    console.log(
      `Deposited: ${amount}. New Account Balance: ${this.account.checkBalance()}`
    );
  }
  ...
  makeWithdrawal(amount: number): void {
    ...
    if (this.account.withdraw(amount)) {
      console.log(
        `Withdrew: ${amount}. New Account Balance: ${this.account.checkBalance()}`
      );
    } else {
  ...
  chargeCreditCard(amount: number): void {
    // Attempt to charge the specified amount to the credit card
    if (this.creditCard.charge(amount)) {
      console.log(
        `Charged: ${amount} to credit card. Current Credit Card Balance: ${this.creditCard.checkCreditCardBalance()}`
      );
    } else {
  ...
  payOffCreditCard(amount: number): void {
    this.creditCard.payOff(amount); // Pay off the specified amount
    console.log(
      `Paid off: ${amount} from credit card. Remaining Balance: ${this.creditCard.checkCreditCardBalance()}`
    );
  }
}
