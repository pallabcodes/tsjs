 ...
 */
export type ApplyNullability<T, TNullability extends JoinNullability> = TNullability extends 'nullable' ? T | null : T;
/**
 ...
 */
export type AppendToNullabilityMap<TNotNullMap extends Record<string, JoinNullability>, TJoinedName extends string, TJoinType extends JoinType> = TJoinType extends 'left' ? TNotNullMap & {
    [name in TJoinedName]: 'nullable';
} : TJoinType extends 'right' ? {
    [Key in keyof TNotNullMap]: 'nullable';
} & {
    [name in TJoinedName]: 'not-null';
} : TJoinType extends 'inner' | 'cross' ? TNotNullMap & {
    [name in TJoinedName]: 'not-null';
