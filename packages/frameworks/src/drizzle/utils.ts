...
type Simplified = Simplify<Intersected>; // { a: 1; b: 2 }

type TestUnion = IsUnion<string | number>; // true
type TestNotUnion = IsUnion<string>; // false

// Example of a type error
type Selection = { id: number; name: string };
type SingleFieldOnly = FromSingleKeyObject<Selection, number, "Selection must have exactly one key">;
// Result: DrizzleTypeError<"Selection must have exactly one key">
