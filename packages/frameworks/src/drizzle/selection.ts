...
export type SelectResultField<T> = 
    ...
    : T extends SQL ? T['_']['type']
    : T extends Table ? SelectResultFields<T['_']['columns']>
...
> = TMode extends 'partial' 
    ? TResult 
    : TMode extends 'single' 
    ? TResult 
    : TResult; // (Simplified: Real Drizzle would handle Join mapping here)
...
const selection = {
    myId: users.id,
    userName: users.name,
    constant: { _: { type: 123 } } as SQL<number>
};

type Result = SelectResultFields<typeof selection>;
/**
 * Result: {
 *   myId: number;
 *   userName: string;
...
// Case B: Table Selection (selecting all columns)
type FullUserResult = SelectResultField<typeof users>;
/**
 * Result: {
 *   id: number;
 *   name: string;
