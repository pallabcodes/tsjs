 ...
 */
export function flattenQueueThenStack(input: unknown[]): unknown[] {
    // Step 1: Breadth-first traversal using a queue
    const queue: unknown[] = [...input]; // Initialize queue with input elements
    const temp: unknown[] = []; // Temporarily holds non-array elements in BFS order
